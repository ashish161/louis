...
This file gives AI coding agents (Claude Code, etc.) the project context they need to work in this repository. It mirrors `AGENTS.md`, which other agents/tools read — keep the two in sync.
...
- `app/` — Vue client. `components/` (subfolders: `myo-editor`, `youtube-picker`, `layout`, `track-art`, `track-trim`, `splash`, `design-system`, `ui`, `desktop`, `dev`), `composables/use*.ts`, `pages/index.vue` (the app) + `pages/design.vue`, `plugins/*.client.ts`, `utils/`.
- `server/` — Nitro server. Auto-imported utils in `server/utils/` (audio/transcode pipeline, yt-dlp, YouTube, Yoto + Spotify OAuth, settings). API routes in `server/api/` (`yoto/`, `youtube/`, `spotify/`, `settings/`, `tools/`). Boot plugins in `server/plugins/`.
- `shared/` — **polyglot-shared** code used by client, server, desktop, and unit tests.
  ...
  - `louis-env.mjs` — the env-binding registry; `louis-settings.mjs` — server-UI credentials file helpers.
  - `.mjs` modules kept out of `server/` so pure-Node unit tests never import Nitro.
- `desktop/` — Electron shell: `main.mjs`, `preload.cjs`, `configStore.mjs`, `js-runtime.mjs` (Electron-as-node yt-dlp shim), `scripts/`. Spawns/builds the same Nitro server, OAuth on port 4010.
...
- Tests compile with plain Node **type stripping** (no esbuild/babel). Runtime-TS syntax that needs a transform is **forbidden**: no `enum`, `namespace`, parameter properties, or decorators. Use `interface`/`type`/`const` unions.
...
- Tests are **colocated** as `*.test.ts` / `*.test.mjs` next to their source.
...
- Reading config in server code: `useRuntimeConfig(event)` (or without event); never read `process.env` directly for these values in server endpoints.
...
- **Applied at the next restart**: non-empty saved fields override `LOUIS_*`/`.env` at boot. Dev: `nuxt.config.ts` top-level `overlayServerSettingsFile()`. Production: the injected nitro preamble re-reads the same file at runtime before runtimeConfig is frozen.
...
- Do not put `settings.json` under `cache/` or `jobs/` — audio-work-dir maintenance sweeps those.

## OAuth flows

- **Yoto**: public PKCE client (`pkce-challenge`). Bundled public client ID: `PK00MDKCVwWvOG8o3px3qSl57FhfUZxm`. Scope set: `offline_access user:content:view user:content:manage user:icons:manage`.
- **Spotify**: OAuth auth-code + client-secret; client ID + secret in env/settings are enough to **paste playlist URLs**; "Connect" (listing playlists) additionally needs a dashboard-registered HTTPS redirect URI (Spotify rejects `localhost`; see `docs/SPOTIFY.md`).
...
- **Save jobs are in-memory / process-local** → Louis must be run **single-replica**.
- yt-dlp sometimes needs `--js-runtimes` (YouTube EJS/nsig): default `node`, desktop sets `node:<Electron shim>` via `desktop/js-runtime.mjs`.
...
- `desktop/main.mjs` is the app entry; spawns or builds the Nitro server (bundled app uses the compiled `.output`), hosts OAuth on port 4010, and passes `LOUIS_PUBLIC_DESKTOP=1`.
- Community binaries (yt-dlp, ffmpeg, spotdl, node) are fetched by `desktop/scripts/fetch-binaries.mjs` and embedded by electron-builder.
- `desktop/configStore.mjs` + `configStore.test.mjs`: persisted user config (Electron `app.getPath`; Settings → Advanced client-ID/redirect handling).

## Release process

See `docs/RELEASE.md` and `.release-it.json`. Version lives in `package.json`; Home Assistant add-on pin is mirrored (`npm run desktop:sync-version`). CHANGELOG via `@release-it/keep-a-changelog`.
